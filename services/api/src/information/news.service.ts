...
const DISASTER_KEYWORDS = [
  ...
  'volcano', 'landslide', 'storm', 'tornado', 'drought', 'extreme heat',
  'extreme cold', 'disaster', 'evacuation', 'emergency', 'rescue',
];
...
export class NewsService {
  ...
  private readonly cache = new Map<string, { expiresAt: number; data: NewsRecord[] }>();

  async find(query: NewsQuery = {}): Promise<NewsRecord[]> {
    const cacheKey = JSON.stringify({ ...query, critical: Boolean(query.critical) });
    ...
    try {
      ...
      if (cached) {
        this.logger.warn(`News provider unavailable; serving stale cache for ${cacheKey}`);
        return cached.data;
      }
      const message = error instanceof Error ? error.message : 'News provider unavailable';
      throw new ServiceUnavailableException(`News data unavailable: ${message}`);
    }
  }
  ...
  private async fetchNews(query: NewsQuery): Promise<NewsRecord[]> {
    ...
    const endpoint = new URL(`https://newsapi.org/v2/${useHeadlines ? 'top-headlines' : 'everything'}`);
    ...
    if (!useHeadlines && !query.query) endpoint.searchParams.set('q', DISASTER_KEYWORDS.join(' OR '));
    ...
    const payload = await response.json() as NewsApiResponse & { message?: string };
    if (!response.ok || payload.status !== 'ok') throw new Error(payload.message || `HTTP ${response.status}`);

    const records = (payload.articles || [])
      .filter((article) => Boolean(article.url && article.title && article.title !== '[Removed]'))
      .map((article) => this.normalize(article, query.category))
      .filter((article) => !query.critical || article.severity === 'CRITICAL' || article.severity === 'HIGH');
    return records;
  }

  normalize(article: NewsApiArticle, category?: string): NewsRecord {
    const text = `${article.title || ''} ${article.description || ''}`.toLowerCase();
    const matches = DISASTER_KEYWORDS.filter((keyword) => text.includes(keyword));
    const severity = matches.length >= 2 ? 'HIGH' : matches.length === 1 ? 'MEDIUM' : 'INFO';
    return {
