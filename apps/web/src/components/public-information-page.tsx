...
export function PublicInformationPage({ title, eyebrow, description, cards }: { title: string; eyebrow: string; description: string; cards: Array<{ title: string; text: string; href?: string }> }) {
  return <main className="public-app">
    <header className="public-header"><Link className="public-brand" href="/"><ShieldCheck /><span>CRISIS<b>MESH</b><small>Resilient Disaster Intelligence</small></span></Link><nav><Link href="/">Home</Link><Link href="/map">Live Map</Link><Link href="/districts">Districts</Link><Link href="/alerts">Alerts</Link><Link href="/news">News</Link><Link href="/safety">Safety</Link><Link href="/resources">Resources</Link><Link href="/about">About</Link></nav><div className="header-actions"><Link href="/login">Login / Sign Up</Link></div></header>
