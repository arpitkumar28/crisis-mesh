...
type Item = { id: string; title?: string; description?: string; url?: string; source?: string; publishedAt?: string; severity?: string; category?: string; temperature?: number; humidity?: number; windSpeed?: number; precipitation?: number; observedAt?: string; is_read?: boolean; priority?: string };
export function InformationFeed({ feed }: { feed: Feed }) {
  ...
  const load = useCallback(async () => { setLoading(true); setError(''); try { const result = await apiClient.get(`/${feed}`); setItems(result.data.data || []); } catch { setError(`Live ${feed} data is unavailable. Check the API connection and retry.`); } finally { setLoading(false); } }, [feed]);
