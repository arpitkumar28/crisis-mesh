...
function coords(item: RecordItem) {
  ...
  const coordinates = Array.isArray(location.coordinates) ? location.coordinates : Array.isArray(item.coordinates) ? item.coordinates : null;
  let resolvedLatitude = latitude;
  let resolvedLongitude = longitude;
  if ((!Number.isFinite(resolvedLatitude) || !Number.isFinite(resolvedLongitude)) && coordinates?.length >= 2) {
    resolvedLongitude = Number(coordinates[0]);
    resolvedLatitude = Number(coordinates[1]);
  }
  // Handle PostGIS POINT format: "POINT(longitude latitude)" - note order is lng, lat
  if ((!Number.isFinite(resolvedLatitude) || !Number.isFinite(resolvedLongitude)) && typeof location === 'string') {
    const point = location.match(/^POINT\s*\(\s*(-?\d+(?:\.\d+)?)\s+(-?\d+(?:\.\d+)?)\s*\)$/i);
    if (point) {
      resolvedLongitude = Number(point[1]); // First coordinate is longitude
      resolvedLatitude = Number(point[2]);  // Second coordinate is latitude
    }
  }
  // Handle nested location object with PostGIS format
  if ((!Number.isFinite(resolvedLatitude) || !Number.isFinite(resolvedLongitude)) && location?.location && typeof location.location === 'string') {
    const point = location.location.match(/^POINT\s*\(\s*(-?\d+(?:\.\d+)?)\s+(-?\d+(?:\.\d+)?)\s*\)$/i);
    if (point) {
      resolvedLongitude = Number(point[1]); // First coordinate is longitude
      resolvedLatitude = Number(point[2]);  // Second coordinate is latitude
    }
  }
  return Number.isFinite(resolvedLatitude) && Number.isFinite(resolvedLongitude) && Math.abs(resolvedLatitude) <= 90 && Math.abs(resolvedLongitude) <= 180 ? { latitude: resolvedLatitude, longitude: resolvedLongitude } : {};
}
...
export default function DashboardPage() {
  ...
  }, [hasHydrated, isAuthenticated, token, router, loadData]);
